...
//  
//      Creation Date :  Thu Dec 20 10:50:48 2012
...
module fifo_manager (clk, rst_b, L1_BCids, packet, fifowr, fifo_full,
                     ...
                     finished, wtdg_rstrt, limit_reached, two_cluster
                     );
  ...
  begin : fifo_manager_WAIT
    ...
    begin
      ...
      case (visual_WAIT_current)  // exemplar parallel_case full_case
        ...
        LD_C1:
          begin
            if (mcluster && ~(limit_reached))
            begin
              next_hit<=1'b0;
              visual_WAIT_current <= REL1;
            end
            else if (~(mcluster) && ~(limit_reached))
            begin
              fifowr<=1'b0;
              next_hit<=1'b0;
              visual_WAIT_current <= STR_FIFO;
